                ...
                <panel style="margin-bottom:0.266667rem;" v-for="son in item.son" :key="son.id">
                  <div slot="body">
                    ...
                    <div class="btn_box" v-if="son.status==1">
                      <x-button  mini v-if="son.all_get_type==0" @click.native="goGetproduct(son.id,son.name,son.get_type,son.conversion_number)" style="margin-right:10px;" type="warn">立即领取</x-button>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      //点击领取礼品页
      goGetproduct(id,name,get_type,conversion_number) {
        this.id =id;
        this.name =name;
      ...
      onConfirm(value) {
        ...
        this.$api.activity.verificationPassword(params).then(res =>{
          ...
          if(res){
            ...
            this.$router.push('/getProduct?id='+this.id+'&name='+this.name+'&get_type='+this.get_type+'&conversion_number='+this.conversion_number)
          }
        })
      },
      onShow() {
